...
<!-- 
	multipart/form-data
	- 모든 문자를 인코딩 하지않고 보내는것을 명시할 때 쓰는 명령어
	...
	- 이 과정에서 바이너리로 보내게 될 데이터를 보낼 수가 없게 되고 type="file"로 처리되는 데이터는 서버로
		전송이 불가능하다.
	- enctype을 multipart/form-data로 변경하게 되면 기존의 데이터 뿐만 아니라 바이너리 데이터 까지 전송이 가능해진다.	
 -->
	...
	<form action="${pageContext.request.contextPath}/test2" method="post" enctype="multipart/form-data">
		<ul>
			<li><h4>select File</h4></li>
			<li><input type="file" name="mediaFile" multiple="multiple"/></li>
			<li><button>Upload</button></li>
		</ul>
	</form>
	...
	<img src="upload/watch.jpg" />	
	<h1>깃허브에 반영할 업데이트 소스ㄱㄴㄴㄱㄴㄱㄴㄱㄴ</h1>
</body>
</html>
